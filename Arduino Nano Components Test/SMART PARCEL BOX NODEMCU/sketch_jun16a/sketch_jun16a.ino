...
void handleRoot() {
  String html = "<html><head><title>Password Reset & Order Numbers</title>";
  html += "<style>body{background-color:#f0f0f0;font-family:Arial,Helvetica,Sans-Serif;margin:0;padding:0;}.container{max-width:600px;margin:50px auto;padding:20px;background-color:#ffffff;border-radius:8px;box-shadow:0 0 10px rgba(0,0,0,0.1);}h1{text-align:center;color:#333333;}form{text-align:center;}input[type='text'],input[type='number']{padding:10px;width:100%;margin-bottom:10px;border:1px solid #ccc;border-radius:4px;box-sizing:border-box;}input[type='submit']{padding:10px 20px;background-color:#4CAF50;color:white;border:none;border-radius:4px;cursor:pointer;}input[type='submit']:hover{background-color:#45a049;}.message{color:#4CAF50;text-align:center;}</style>";
  html += "<script>function submitPassword(){var xhr=new XMLHttpRequest();var password=document.getElementById('password').value;xhr.open('GET','/password?password='+password);xhr.onload=function(){if(xhr.status===200){document.getElementById('password').value='';document.getElementById('password-message').innerHTML='Password updated successfully!';}else{document.getElementById('password-message').innerHTML='Failed to update password. Please try again.';}};xhr.send();}function submitOrder(orderNum){var xhr=new XMLHttpRequest();var order=document.getElementById('order'+orderNum).value;xhr.open('GET','/submit'+orderNum+'?order='+order);xhr.onload=function(){if(xhr.status===200){document.getElementById('order'+orderNum).value='';document.getElementById('order'+orderNum+'-message').innerHTML='Order No. '+orderNum+' updated successfully!';}else{document.getElementById('order'+orderNum+'-message').innerHTML='Failed to update Order No. '+orderNum+'. Please try again.';}};xhr.send();}</script>";
  html += "</head><body><div class='container'>";
  html += "<h1>Password Reset</h1><form onsubmit='event.preventDefault(); submitPassword();'>Enter 4-digit password:<br><input type='text' id='password' name='password' pattern='\\d{4}' title='Please enter exactly 4 digits' required><br><br><input type='submit' value='Submit'><p id='password-message' class='message'></p></form>";
  html += "<h1>Order Numbers</h1><form onsubmit='event.preventDefault(); submitOrder(1);'>Order No. 1:<br><input type='number' id='order1' name='order' min='0' max='99999' value='" + order1 + "' required><br><br><input type='submit' value='Submit'><p id='order1-message' class='message'></p></form>";
